...
from .dataset import Places365Dataset, Cifar10Dataset, TestDataset, VideoDAVIS
...
class BaseModel:
    def __init__(self, sess, options):
        ...
        self.test_log_file = os.path.join(options.checkpoints_path, 'log_test.txt')
        self.valid_log_file = os.path.join(options.checkpoints_path, 'log_valid.txt')
        self.train_log_file = os.path.join(options.checkpoints_path, 'log_train.txt')
...
class VideoDAVISModel(BaseModel):
    def __init__(self, sess, options):
        super(VideoDAVISModel, self).__init__(sess, options)
    ...
    def create_dataset(self, training=True):
        return VideoDAVIS(
            ...
            training=training,
            augment=self.options.augment)
    ## end of 256 256 model ##

    def test(self):
        print('\nTesting on DAVIS')
        dataset = VideoDAVIS("")
        outputs_path = create_dir("./output")

        for index in range(len(dataset)):
            img_gray = dataset[index]
            ...
            imsave(outputs[0], path)
            #save rgb
            imsave(dataset[index], "./output/original.jpg")
            imsave(img_gray, "./output/gray.jpg")
            break
